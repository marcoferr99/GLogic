...
Definition clause_f_type := list ty -> nat -> option (prod def_type gtm).
Definition clause_f_spec (clause_f : clause_f_type) :=
  forall C l n t B, clause_f l n = Some (t, B) ->
...
Theorem has_type_clause {L n t B C T} {cf : clause_f_type} :
  clause_f_spec cf ->
  cf L n = Some (t, B) ->
  has_type C B T <->
...
Proof.
  ...
  - simpl in *. subst. eapply H. eassumption.
Qed.

Theorem has_type_clause_app {L n t B C x} {cf : clause_f_type} :
  clause_f_spec cf ->
  let p := gtm_prd L n in
  cf L n = Some (x, B) ->
  Forall2 (has_type C) t L ->
  is_form C (foldl tm_app <{ B p }> t).
Proof.
  intros. has_type. exists L > [|assumption].
  has_type_goal (). eexists > [|now constructor].
  eapply has_type_clause; eauto.
Qed.

Ltac2 ht_clause h :=
  let c := Control.hyp h in
  match! goal with
  | [ o : _ _ _ = Some _, sp : clause_f_spec _ |- _ ] =>
      let spc := Control.hyp sp in
      let oc := Control.hyp o in let a := Fresh.in_goal @H in
      assert ($a := proj1 (has_type_clause $spc $oc) $c); simpl in $a; try (ty_injection $a); []
  end.
...
Ltac2 htg_clause () :=
  ...
  | [ o : _ _ _ = Some _, sp : clause_f_spec _ |- _ ] =>
      ...
      refine '(proj2 (has_type_clause $spc $oc) _); try reflexivity
  end.

Ltac2 htg_clause_app () :=
  match! goal with
  | [ o : _ _ _ = Some _, sp : clause_f_spec _ |- _ ] =>
      let spc := Control.hyp sp in
      let oc := Control.hyp o in
      refine '(has_type_clause_app $spc $oc _)
  end.

Ltac2 Set htg_list as old := fun () => List.append [htg_clause; htg_clause_app] (old ()).
...
Inductive is_derivable_wf (cf : clause_f_type) : context -> tm_set -> tm -> Prop :=
  | wrl_set C l m c :
      l ≡ m ->
      is_derivable_wf cf C l c ->
      is_derivable_wf cf C m c
  | wrl_id C a b :
      has_type C a ty_prp ->
      has_type C b ty_prp ->
      tm_equiv C a b ->
      is_derivable_wf cf C {[a]} b
  | wrl_cut C l m b c :
      is_derivable_wf cf C l b ->
      is_derivable_wf cf C (m +; b) c ->
      is_derivable_wf cf C (l ∪ m) c
  | wrl_cL C l b c :
      is_derivable_wf cf C (l +; b +; b) c ->
      is_derivable_wf cf C (l +; b) c
  | wrl_wL C l m b :
      (forall a, a ∈ l -> has_type C a ty_prp) ->
      is_derivable_wf cf C m b ->
      is_derivable_wf cf C (l ∪ m) b
  | wrl_botL C c :
      has_type C c ty_prp ->
      is_derivable_wf cf C {[gtm_bot]} c
  | wrl_topR C :
      is_derivable_wf cf C ∅ gtm_top
  | wrl_orL C l b c d :
      is_derivable_wf cf C (l +; b) c ->
      is_derivable_wf cf C (l +; d) c ->
      is_derivable_wf cf C (l +; <{ b \/ d }>) c
  | wrl_orR1 C l b c :
      has_type C c ty_prp ->
      is_derivable_wf cf C l b ->
      is_derivable_wf cf C l <{ b \/ c }>
  ...
  | wrl_forL C T l b c t :
      has_type C t T ->
      is_derivable_wf cf C (l +; subst_last C C t b) c ->
      is_derivable_wf cf C (l +; <{for T, b}>) c
  | wrl_forR C T l b sb :
      (forall t, t ∈ sb <-> in_supp b t) ->
      is_derivable_wf cf (C +: foldr ty_arr T (type_check_other <$> sb)) (tm_set_map (lift C) l) (
        subst_last C (S C) (foldl tm_app (tm_lvl C) sb) b
      ) ->
      is_derivable_wf cf C l <{for T, b}>
  | wrl_exL C T l b c sb :
      (forall t, t ∈ sb <-> in_supp b t) ->
      is_derivable_wf cf (C +: foldr ty_arr T (type_check_other <$> sb)) (
        tm_set_map (lift C) l ∪
        {[subst_last C (S C) (foldl tm_app (tm_lvl C) sb) b]}
      ) (lift C c) ->
      is_derivable_wf cf C (l +; <{ex T, b}>) c
  | wrl_exR C T l b t :
      has_type C t T ->
      is_derivable_wf cf C l (subst_last C C t b) ->
      is_derivable_wf cf C l <{ex T, b}>
  | wrl_nabL {T b} n (_ : ~ in_supp b (gtm_nom T n)) C l c :
      is_derivable_wf cf C (l +; subst_last C C (gtm_nom T n) b) c ->
      is_derivable_wf cf C (l +; <{nab T, b}>) c
  | wrl_nabR {T b} n (_ : ~ in_supp b (gtm_nom T n)) C l :
      is_derivable_wf cf C l (subst_last C C (gtm_nom T n) b) ->
      is_derivable_wf cf C l (<{nab T, b}>)
  | wrl_defL C l L n x B c t :
      let p := gtm_prd L n in
      cf L n = Some (x, B) ->
      is_derivable_wf cf C (l +; foldl tm_app <{B p}> t) c ->
      is_derivable_wf cf C (l +; foldl tm_app p t) c
  | wrl_defR C l L n x B t :
      let p := gtm_prd L n in
      cf L n = Some (x, B) ->
      is_derivable_wf cf C l (foldl tm_app <{B p}> t) ->
      is_derivable_wf cf C l (foldl tm_app p t)
  | wrl_iL C l L n B s t c :
      let p := gtm_prd L n in
      cf L n = Some (def_ind, B) ->
      (forall D, has_type D s (foldr ty_arr ty_prp L)) ->
      let x := (map tm_lvl (seq 0 (length L))) in
      is_derivable_wf cf (list_to_rlist L) {[foldl tm_app <{B s}> x]} (foldl tm_app s x) ->
      is_derivable_wf cf C (l +; foldl tm_app s t) c ->
      is_derivable_wf cf C (l +; foldl tm_app p t) c
  | wrl_ciR C l L n B s t :
      let p := gtm_prd L n in
      cf L n = Some (def_cin, B) ->
      (forall D, has_type D s (foldr ty_arr ty_prp L)) ->
      let x := (map tm_lvl (seq 0 (length L))) in
      is_derivable_wf cf (list_to_rlist L) {[foldl tm_app s x]} (foldl tm_app <{B s}> x) ->
      is_derivable_wf cf C l (foldl tm_app s t) ->
      is_derivable_wf cf C l (foldl tm_app p t)
.

Inductive is_derivable (cf : clause_f_type) : context -> tm_set -> tm -> Prop :=
  | rl_set C l m c :
      l ≡ m ->
      is_derivable cf C l c ->
      is_derivable cf C m c
  | rl_id (C : context) l a b :
      tm_equiv C a b ->
      is_derivable cf C (l +; a) b
  | rl_cut C l m b c :
      has_type C b ty_prp ->
      is_derivable cf C l b ->
      is_derivable cf C (m +; b) c ->
      is_derivable cf C (l ∪ m) c
  | rl_cL C l b c :
      is_derivable cf C (l +; b +; b) c ->
      is_derivable cf C (l +; b) c
  | rl_botL C l c :
      is_derivable cf C (l +; gtm_bot) c
  | rl_topR C l :
      is_derivable cf C l gtm_top
  | rl_orL C l b c d :
      is_derivable cf C (l +; b) c ->
      is_derivable cf C (l +; d) c ->
      is_derivable cf C (l +; <{ b \/ d }>) c
  | rl_orR1 C l b c :
      is_derivable cf C l b ->
      is_derivable cf C l <{ b \/ c }>
  | rl_orR2 C l b c :
  ...
  | rl_forL C T l b c t :
      has_type C t T ->
      is_derivable cf C (l +; subst_last C C t b) c ->
      is_derivable cf C (l +; <{for T, b}>) c
  | rl_forR C T l b sb :
      (forall t, t ∈ sb <-> in_supp b t) ->
      is_derivable cf (C +: foldr ty_arr T (type_check_other <$> sb)) (tm_set_map (lift C) l) (
        subst_last C (S C) (foldl tm_app (tm_lvl C) sb) b
      ) ->
      is_derivable cf C l <{for T, b}>
  | rl_exL C T l b c sb :
      (forall t, t ∈ sb <-> in_supp b t) ->
      is_derivable cf (C +: foldr ty_arr T (type_check_other <$> sb)) (
        tm_set_map (lift C) l ∪
        {[subst_last C (S C) (foldl tm_app (tm_lvl C) sb) b]}
      ) (lift C c) ->
      is_derivable cf C (l +; <{ex T, b}>) c
  | rl_exR C T l b t :
      has_type C t T ->
      is_derivable cf C l (subst_last C C t b) ->
      is_derivable cf C l <{ex T, b}>
  | rl_nabL {T b} n (_ : ~ in_supp b (gtm_nom T n)) C l c :
      is_derivable cf C (l +; subst_last C C (gtm_nom T n) b) c ->
      is_derivable cf C (l +; <{nab T, b}>) c
  | rl_nabR {T b} n (_ : ~ in_supp b (gtm_nom T n)) C l :
      is_derivable cf C l (subst_last C C (gtm_nom T n) b) ->
      is_derivable cf C l (<{nab T, b}>)
  | rl_defL C l L n x B c t :
      let p := gtm_prd L n in
      cf L n = Some (x, B) ->
      is_derivable cf C (l +; foldl tm_app <{B p}> t) c ->
      is_derivable cf C (l +; foldl tm_app p t) c
  | rl_defR C l L n x B t :
      let p := gtm_prd L n in
      cf L n = Some (x, B) ->
      is_derivable cf C l (foldl tm_app <{B p}> t) ->
      is_derivable cf C l (foldl tm_app p t)
  | rl_iL C l L n B s t c :
      let p := gtm_prd L n in
      cf L n = Some (def_ind, B) ->
      (forall D, has_type D s (foldr ty_arr ty_prp L)) ->
      let x := (map tm_lvl (seq 0 (length L))) in
      is_derivable cf (list_to_rlist L) {[foldl tm_app <{B s}> x]} (foldl tm_app s x) ->
      is_derivable cf C (l +; foldl tm_app s t) c ->
      is_derivable cf C (l +; foldl tm_app p t) c
  | rl_ciR C l L n B s t :
      let p := gtm_prd L n in
      cf L n = Some (def_cin, B) ->
      (forall D, has_type D s (foldr ty_arr ty_prp L)) ->
      let x := (map tm_lvl (seq 0 (length L))) in
      is_derivable cf (list_to_rlist L) {[foldl tm_app s x]} (foldl tm_app <{B s}> x) ->
      is_derivable cf C l (foldl tm_app s t) ->
      is_derivable cf C l (foldl tm_app p t)
.

Notation "[{ sig ; l --> B }]" := (is_derivable (fun _ _ => None) sig l B)
  (sig custom stlc_ty, l custom stlc_tm, B custom stlc_tm) : stlc_scope.

Notation "({ D ; sig ; l --> B })" := (is_derivable D sig l B)
  (sig custom stlc_ty, l custom stlc_tm, B custom stlc_tm) : stlc_scope.
...
Theorem is_derivable_wf_wf cf C l c : clause_f_spec cf -> is_derivable_wf cf C l c -> wf_sequent C l c.
Proof.
  intros CS D. unfold wf_sequent. induction D; simpl in *.
  ...
  - set_unfold. intuition.
    ...
    eapply foldr_ty_arr_inj. eassumption.
  - set_unfold. intuition. subst.
    assert (is_form C (foldl tm_app s t)) by auto.
    has_type. exists L > [has_type_goal ()|].
    ltac1:(replace L with x1) > [assumption|].
    eapply foldr_ty_arr_inj.
    eapply has_type_unique; eauto.
  - set_unfold. intuition.
    has_type. exists L > [has_type_goal ()|].
    ltac1:(replace L with x2) > [assumption|].
    eapply foldr_ty_arr_inj.
    eapply has_type_unique; eauto.
Qed.

Theorem rl_wL D C l m c :
  ({ D; C; m --> c }) -> ({ D; C; l ∪ m --> c }).
Proof.
  revert C l m c.
...
Theorem is_derivable_wf_is_derivable cf C l c : clause_f_spec cf ->
  is_derivable_wf cf C l c -> ({ cf; C; l --> c }).
Proof.
  intros CS DW. induction DW; subst.
  - eapply rl_set; eassumption.
  - rewrite <- union_empty. now apply rl_id.
  - eapply rl_cut; try eassumption.
    apply is_derivable_wf_wf in DW1.
    + destruct DW1. assumption.
    + assumption.
  - now apply rl_cL.
  - now apply rl_wL.
  ...
  - eapply rl_iL; eassumption.
  - eapply rl_ciR; eassumption.
Qed.

Theorem is_derivable_is_derivable_wf cf C l c : clause_f_spec cf ->
  wf_sequent C l c ->  ({ cf; C; l --> c }) -> is_derivable_wf cf C l c.
Proof.
  intros CS [] D. unfold wf_tm_set in *. induction D; set_unfold.
  ...
  - eapply wrl_defL > [eassumption|].
    apply IHD; intuition. subst.
    assert (is_form C (foldl tm_app p t)) by auto. has_type.
    ltac1:(replace L with x0) > [assumption|].
    eapply foldr_ty_arr_inj. eassumption.
  - eapply wrl_defR > [eassumption|].
    apply IHD; intuition. has_type.
    ltac1:(replace L with x0) > [assumption|].
    eapply foldr_ty_arr_inj. eassumption.
  - eapply wrl_iL; try eassumption.
    + apply IHD1.
      * intros. subst.
        has_type. exists L.
        -- has_type_goal (). eexists > [has_type_goal ()|].
           apply H2.
        -- subst x. apply has_type_lvl_seq.
      * has_type. exists L > [apply H2|].
        subst x. apply has_type_lvl_seq.
    + apply IHD2 > [|assumption].
      intuition. subst. has_type.
      exists L > [apply H2|].
      assert (is_form C (foldl tm_app p t)) by auto.
      has_type.
      apply foldr_ty_arr_inj in H4. congruence.
  - eapply wrl_ciR; try eassumption.
    + apply IHD1.
      * intros. subst.
        has_type. exists L > [apply H2|].
        subst x. apply has_type_lvl_seq.
    ...
    + apply IHD2 > [assumption|].
      has_type.
      exists L > [apply H2|].
      apply foldr_ty_arr_inj in H3. congruence.
Qed.


Theorem rl_nabL2 cf C T b l c :
...
Theorem contrapositive D C l b c :
  ({ D; C; l +; b --> c }) -> ({ D; C; l +; ~ c --> ~ b }).
Proof.
  intros H.
...
Theorem rl_id2 cf C l t : is_derivable cf C (l ∪ {[t]}) t.
